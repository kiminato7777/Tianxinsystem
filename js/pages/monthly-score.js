...
const subjectKeysGK = [
    'gk_kh_dict', 'gk_kh_write', 'gk_kh_read', 'gk_kh_speak', 
    'gk_math_num', 'gk_math_meas', 'gk_math_geo', 'gk_math_alg', 
    'gk_science', 'gk_social', 'gk_pe_sports', 'gk_pe_health', 'gk_arts'
];
...
function renderTable() {
    ...
    if (headerRow) {
        ...
        // Update headers to support up to 13 columns
        for (let i = 1; i <= 13; i++) {
            const label = isGeneralOnly ? generalLabels[i-1] : chineseLabels[i-1];
            if (headers[i]) {
                headers[i].innerText = label || '-';
                headers[i].title = label || '-';
                headers[i].style.display = label === '-' ? 'none' : 'table-cell';
            }
        }
    }
    ...
    body.innerHTML = filteredStudents.map((s, idx) => {
        ...
        return `
            <tr data-key="${s.key}" data-program="${pType}" class="${isCompleted ? 'completed' : ''}">
                ...
                ${(isCh ? subjectKeysCH : subjectKeysGK).map((key, kIdx) => {
                    return `
                    <td>
                        <input type="number" step="0.01" min="0" max="10" 
                            class="score-input input-${key}" 
                            data-key="${key}" 
                            value="${record[key] !== undefined ? record[key] : ''}"
...
function recalculateRow(tr) {
    ...
    // Dynamic Divider: 11 for Chinese, 13 for General Knowledge
    const divider = pType === 'chinese-fulltime' ? 11 : 13;
...
async function saveAllScores() {
    ...
    try {
        ...
        for (const row of rows) {
            ...
            // Get component scores based on program
            const activeKeys = pType === 'chinese-fulltime' ? subjectKeysCH : subjectKeysGK;
            activeKeys.forEach(sKey => {
                const inputEl = row.querySelector(`.input-${sKey}`);
...
function exportToExcel() {
    ...
    // Adjust headers based on program if filtered
    const programFilter = document.getElementById('filterProgram').value;
    if (programFilter === 'general') {
        headers.splice(4, 11, ...generalLabels);
    } else {
        headers.splice(4, 11, ...chineseLabels.slice(0, 11));
    }
    data.push(headers);
    ...
    rows.forEach((row, idx) => {
        ...
        const rowData = [
            ...
            student.studyTime || '',
            ...(student.programType === 'general' || (student.studyType || '').toLowerCase().includes('general') ? subjectKeysGK : subjectKeysCH).map(k => row.querySelector(`.input-${k}`)?.value || '0'),
