...
window.updateHybridPreview = function () {
    ...
    // Update Stage in Preview
    const stageEl = document.getElementById('payStage');
    const summaryStageEl = document.getElementById('summary-stage');
    if (stageEl && summaryStageEl) {
        summaryStageEl.innerText = stageEl.value ? `លើកទី ${stageEl.value}` : '---';
    }
    ...
    // User Requirement: Amount to Pay - Paid = Remaining Debt
    const active = document.activeElement;

    if (active === boardingEl) {
        // Typing in "Remaining Debt" -> Calculate Paid Amount
        const gap = parseFloat(boardingEl.value) || 0;
        actualPaid = amount - gap;
        actualPaidInput.value = (actualPaid >= 0) ? actualPaid.toFixed(2) : "0.00";
    } else {
        // Automatically sync Remaining Debt based on Amount and Actual Paid (covers typing in Amount, Paid, or clicking buttons)
    ...
    // --- 1.5 AUTO STATUS UPDATE ---
    const statusSelect = document.getElementById('payStatus');
    const postponedContainer = document.getElementById('postponedDateContainer');
    if (statusSelect && (active === actualPaidInput || active === boardingEl || active === amountInput)) {
        if (boarding > 0) {
            statusSelect.value = 'Installment';
        } else if (actualPaid >= amount && amount > 0) {
            statusSelect.value = 'Paid';
        }
        
        if (statusSelect.value === 'Delay' || statusSelect.value === 'Installment') { 
            if (postponedContainer) postponedContainer.style.display = 'block'; 
        } else { 
            if (postponedContainer) postponedContainer.style.display = 'none'; 
        }
    }
...
window.calculateHybridOutcome = function () {
    ...
    let addedMonths = parseFloat(payTotalMonthsInput?.value);

    // On first load or if empty, default to 0
    if (isNaN(addedMonths)) addedMonths = 0;

    const _paidMonths = addedMonths;
    const currentTotalMonths = prevTotalMonths + addedMonths;
...
async function showAdditionalPaymentModal(key) {
    ...
    // Auto-calculate next stage
    let currentCount = 0;
    let maxStage = 0;
    const initialAmt = parseFloat(s.initialPayment) || 0;
    if (initialAmt > 0) {
        currentCount = 1;
        maxStage = 1;
    }
    if (s.installments) {
        ...
        let actualInstCount = 0;
        instList.forEach(inst => {
            // Skip initial payment duplicate installments
            if (initialAmt > 0 && (inst.stage == 1 || inst.stage == '1' || inst.isInitial)) return;
            const stg = parseInt(inst.stage) || 0;
            if (stg > maxStage) maxStage = stg;
            actualInstCount++;
        });
        currentCount += actualInstCount;
    }
    const autoStage = Math.max(currentCount + 1, maxStage + 1);
            ...
            <div class="modal-content ultra-v5-premium">
                ...
                <div class="modal-body p-0">
                    <div class="row g-0">
                        ...
                        <div class="col-lg-7 p-4 p-xl-5 entry-panel shadow-sm" style="background: #f8fafc;">
                            <form id="additionalPaymentForm">
                                ...
                                <!-- SECTION 1: ព័ត៌មានទូទៅ (GENERAL INFO) -->
                                <div class="v5-card-section mb-4 animate__animated animate__fadeInUp" style="animation-delay: 0.1s;">
                                    <div class="d-flex align-items-center mb-3 px-1">
                                        <div class="v5-section-icon bg-primary bg-opacity-10 text-primary me-2"><i class="fi fi-rr-user"></i></div>
                                        <h5 class="fw-bold mb-0 text-dark">ព័ត៌មានទូទៅ (General Info)</h5>
                                    </div>
                                    <div class="bg-white p-4 rounded-4 shadow-sm border border-secondary border-opacity-10">
                                        <!-- STUDENT BADGE -->
                                        <div class="d-flex align-items-center w-100 mb-4 pb-4 border-bottom">
                                            ...
                                            <div class="v5-profile-info flex-grow-1 ms-3">
                                                <h4 class="v5-student-name mb-1">${studentName}</h4>
                                                <div class="text-muted small fw-bold"><i class="fi fi-rr-id-badge me-1"></i>ID: ${s.displayId || '---'} &nbsp;•&nbsp; <i class="fi fi-rr-graduation-cap me-1"></i>${s.studyLevel || 'មិនមាន'}</div>
                                            </div>
                                        </div>
                                        
                                        <!-- GRID: DATE, METHOD, RECEIVER -->
                                        <div class="row g-4">
                                            <div class="col-md-6">
                                                <div class="v5-group">
                                ...
                                <!-- SECTION 2: ហិរញ្ញវត្ថុ (FINANCIAL DETAILS) -->
                                <div class="v5-card-section mb-4 animate__animated animate__fadeInUp" style="animation-delay: 0.2s;">
                                    <div class="d-flex align-items-center mb-3 px-1">
                                        <div class="v5-section-icon bg-emerald bg-opacity-10 text-emerald me-2"><i class="fi fi-rr-sack-dollar"></i></div>
                                        <h5 class="fw-bold mb-0 text-dark">ហិរញ្ញវត្ថុ (Financial Details)</h5>
                                    </div>
                                    <div class="bg-white p-4 rounded-4 shadow-sm border border-secondary border-opacity-10">
                                        <div class="row g-4 mb-4">
                                            <div class="col-md-4">
                                                <div class="v5-group h-100 d-flex flex-column">
                                                    <label class="v5-label">ប្រាក់ត្រូវបង់ (AMOUNT TO PAY)</label>
                                                    <div class="v5-field-box mt-auto bg-light border-0">
                                            ...
                                            <div class="col-md-4">
                                                <div class="v5-group active-emerald h-100 d-flex flex-column">
                                                    <label class="v5-label text-emerald">ប្រាក់បានបង់ (ACTUAL PAID)</label>
                                                    <div class="v5-field-box emerald-variant mt-auto border-0">
                                            ...
                                            <div class="col-md-4">
                                                <div class="v5-group h-100 d-flex flex-column">
                                                    <label class="v5-label text-danger">នៅជំពាក់ (REMAINING DEBT)</label>
                                                    <div class="v5-field-box mt-auto border-0" style="background: #fff1f2;">
                                                        <span class="v5-field-cur text-danger">$</span>
                                                        <input type="number" step="0.01" id="payBoardingFee" class="v5-input text-danger fw-black" placeholder="0.00" oninput="window.updateHybridPreview()">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <div class="v5-shortcuts mb-4">
                                        ...
                                        <div class="row g-4 pt-3 border-top">
                                            <div class="col-md-6">
                                                <div class="v5-group">
                                                    ...
                                                    <div class="v5-field-box soft-bg" style="height: auto; padding: 4px;">
                                                ...
                                                <div class="v5-group animate__animated animate__fadeIn p-3 rounded-4 border-start border-4 border-warning bg-warning bg-opacity-5">
                                                    <div class="row g-3">
                                                        <div class="col-md-6">
                                                            ...
                                                            <div class="v5-field-box bg-white shadow-sm border-0">
                                        ...
                                        <!-- ADVANCED COLLAPSE (FEES) -->
                                        <div class="v5-accordion mt-4 border-0 bg-light rounded-4">
                                            <div class="v5-acc-header rounded-4" data-bs-toggle="collapse" href="#v5Fees" style="background: transparent;">
                                                <span class="fw-bold text-primary"><i class="fi fi-rr-settings-sliders me-2"></i>បន្ថែមការបញ្ចុះតម្លៃ និង សេវាផ្សេងៗ</span>
                                                <i class="fi fi-rr-angle-small-down"></i>
                                            </div>
                                            <div class="collapse show" id="v5Fees">
                                                <div class="v5-acc-body p-4 pt-0">
                                                    <div class="grid-3-v5" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px;">
                                                        <div class="v5-mini-field">
                                                            <label>បញ្ចុះ %</label>
                                                            <input type="number" id="payDiscountPercent" placeholder="0" class="bg-white border-0 shadow-sm" oninput="window.updateHybridPreview()">
                                                        </div>
                                                        <div class="v5-mini-field">
                                                            <label>បញ្ចុះ $</label>
                                                            <input type="number" id="payDiscountDollar" placeholder="0.00" class="bg-white border-0 shadow-sm" oninput="window.updateHybridPreview()">
                                                        </div>
                                                        <div class="v5-mini-field">
                                                            <label>សេវាផ្សេងៗ</label>
                                                            <input type="number" id="payAdminFee" placeholder="0.00" class="bg-white border-0 shadow-sm" oninput="window.updateHybridPreview()">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <!-- SECTION 3: រយៈពេលសិក្សា (STUDY DURATION) -->
                                <div class="v5-card-section mb-4 animate__animated animate__fadeInUp" style="animation-delay: 0.3s;">
                                    <div class="d-flex align-items-center mb-3 px-1">
                                        <div class="v5-section-icon bg-blue bg-opacity-10 text-blue me-2"><i class="fi fi-rr-time-forward"></i></div>
                                        <h5 class="fw-bold mb-0 text-dark">រយៈពេលសិក្សា (Timeline)</h5>
                                    </div>
                                    <div class="bg-white p-4 rounded-4 shadow-sm border border-secondary border-opacity-10">
                                        <div class="row g-4 align-items-center">
                                            <div class="col-md-4">
                                                <span class="v5-micro-label text-purple" style="color: #8b5cf6;">លើកទី (STAGE)</span>
                                                <div class="d-flex align-items-center gap-2 mt-1 fw-bold" style="color: #8b5cf6;">
                                                    <i class="fi fi-rr-layer-plus fs-4"></i>
                                                    <input type="number" id="payStage" class="v5-ghost-input w-100 border-bottom pb-1" style="color: inherit; border-color: rgba(139,92,246,0.3) !important;" value="${autoStage}" placeholder="Auto" oninput="window.updateHybridPreview()">
                                                </div>
                                            </div>
                                            <div class="col-md-4 v5-divider-left ps-4">
                                                <span class="v5-micro-label text-blue">ចំនួនខែ (MONTHS)</span>
                                                <div class="d-flex align-items-center gap-2 mt-1 text-blue fw-bold">
                                                    <i class="fi fi-rr-time-forward fs-4"></i>
                                                    <input type="number" id="payTotalMonths" class="v5-ghost-input w-100 border-bottom pb-1" style="color: inherit; border-color: rgba(59,130,246,0.3) !important;" oninput="window.updateHybridPreview()" placeholder="0">
                                                </div>
                                            </div>
                                            <div class="col-md-4 v5-divider-left ps-4">
                                                <span class="v5-micro-label text-emerald">ថ្ងៃផុតកំណត់ថ្មី (NEW DUE)</span>
                                                <div class="d-flex align-items-center gap-2 mt-1 text-emerald fw-bold">
                                                    <i class="fi fi-rr-calendar-clock fs-4"></i>
                                                    <input type="text" id="payNextDueDate" class="v5-ghost-input w-100 border-bottom pb-1" style="color: inherit; border-color: rgba(16,185,129,0.3) !important;" placeholder="Select date...">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <!-- SECTION 4: ផ្សេងៗ (OTHERS) -->
                                <div class="v5-card-section animate__animated animate__fadeInUp" style="animation-delay: 0.4s;">
                                    <div class="d-flex align-items-center mb-3 px-1">
                                        <div class="v5-section-icon bg-secondary bg-opacity-10 text-secondary me-2"><i class="fi fi-rr-clip"></i></div>
                                        <h5 class="fw-bold mb-0 text-dark">ឯកសារ & សម្គាល់ (Attachment & Note)</h5>
                                    </div>
                                    <div class="bg-white p-4 rounded-4 shadow-sm border border-secondary border-opacity-10">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            <label class="v5-label mb-0"><i class="fi fi-rr-picture me-2 text-primary"></i>វិក្កយបត្រ (RECEIPT)</label>
                                        ...
                                        <div id="receiptPreviewContainer" class="d-flex flex-wrap gap-2 mb-4">
                                            <div class="v5-upload-placeholder w-100 border-2 border-dashed border-secondary opacity-50 rounded-4 p-4 text-center">
                                                <i class="fi fi-rr-cloud-upload fs-1 d-block mb-2 text-muted"></i>
                                                <p class="small text-muted fw-bold mb-0">មិនទាន់មានរូបភាព</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mt-2 border-top pt-3">
                                            ...
                                            <textarea id="payNote" class="v5-textarea bg-light border-0 shadow-sm" placeholder="បញ្ចូលព័ត៌មានបន្ថែម..." style="height: 60px;"></textarea>
                                        </div>
                                    </div>
                                </div>
                            </form>
                        ...
                        <div class="col-lg-5 summary-panel p-4 p-xl-5" style="background: #ffffff; border-left: 1px solid #e2e8f0;">
                            <div class="v5-receipt-sticky position-sticky" style="top: 20px;">
                                <div class="v5-receipt shadow-premium animate__animated animate__fadeInRight" style="border: 1px solid #e2e8f0; border-radius: 24px; overflow: hidden; background: #fff; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.15);">
                                    <div class="v5-receipt-header py-4 px-4" style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%);">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            <div class="text-white opacity-75 small fw-bold">RECEIPT PREVIEW</div>
                                            <div class="v5-receipt-seal" style="position: static; transform: none; width: 32px; height: 32px; border: 2px solid rgba(255,255,255,0.2); background: rgba(255,255,255,0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                                                <i class="fi fi-rr-star text-white" style="font-size: 12px;"></i>
                                            </div>
                                        </div>
                                        <h3 class="v5-receipt-brand text-white mb-0 fs-5" style="letter-spacing: 1px; font-weight: 900;">TIAN XIN INTERNATIONAL</h3>
                                    </div>
                                    
                                    <div class="v5-receipt-body px-4 py-4">
                                        <!-- MAIN CENTERPIECE -->
                                        <div class="v5-centerpiece pb-4 mb-4" style="border-bottom: 2px dashed #e2e8f0; text-align: center;">
                                            <span class="v5-center-label text-uppercase fw-bold text-muted mb-2 d-block" style="font-size: 0.8rem;">ប្រាក់បង់សរុប (TOTAL PAID)</span>
                                            <div class="v5-center-amount d-flex align-items-center justify-content-center gap-1">
                                                <span class="v5-center-cur text-dark opacity-50 fs-4 fw-bold">$</span>
                                                <span id="summary-total" class="v5-center-val text-dark fw-black" style="font-size: 3.5rem; letter-spacing: -2px;">0.00</span>
                                            </div>
                                            <div class="v5-center-riel text-muted fw-bold" id="summary-riel" style="font-size: 1.1rem; font-family: 'Kantumruy Pro', sans-serif;">0 ៛</div>
                                            <div class="mt-3 text-center">
                                                <span id="summary-status-badge" class="badge-status-glow active-partial d-inline-flex align-items-center px-3 py-2 rounded-pill fw-bold" style="background: #f8fafc; border: 1px solid #e2e8f0; color: #64748b; font-size: 0.75rem;"><i class="fi fi-rr-info me-2"></i>WAITING FOR ENTRY</span>
                                            </div>
                                        </div>

                                        <!-- DETAILS -->
                                        <div class="v5-details">
                                            <div class="d-flex justify-content-between mb-3 fw-bold text-secondary" style="font-size: 0.95rem;">
                                                <span>បង់លើកទី (Stage)</span>
                                                <span id="summary-stage" class="text-purple fw-black">លើកទី ${autoStage}</span>
                                            </div>
                                            <div class="d-flex justify-content-between mb-3 fw-bold text-secondary" style="font-size: 0.95rem;">
                                                <span>តម្លៃសិក្សា (Tuition)</span>
                                                <span id="summary-subtotal" class="text-dark fw-black">$0.00</span>
                                            </div>
                                            <div class="d-flex justify-content-between mb-4 fw-bold" style="font-size: 0.95rem;">
                                                <span class="text-secondary">សេវា & បញ្ចុះតម្លៃ</span>
                                                <span id="summary-adjustments" class="text-emerald fw-black">$0.00</span>
                                            </div>
                                            
                                            <div class="v5-outcome-card p-3 rounded-4 mb-4" style="background: #f0fdf4; border: 1px solid #dcfce7;">
                                                <div class="d-flex align-items-center gap-3">
                                                    <div class="bg-white p-2 rounded-3 shadow-sm text-emerald d-flex align-items-center justify-content-center" style="width: 40px; height: 40px;">
                                                        <i class="fi fi-rr-calendar-check fs-5"></i>
                                                    </div>
                                                    <div>
                                                        <span class="text-uppercase text-emerald opacity-75 fw-bold d-block" style="font-size: 0.7rem;">ថ្ងៃផុតកំណត់ថ្មី (NEW DUE)</span>
                                                        <span id="summary-next-date" class="fw-black text-emerald d-block fs-6">--- --- ---</span>
                                                    </div>
                                                </div>
                                            </div>

                                            <!-- REMAINING BOX -->
                                            <div class="remaining-footer p-4 rounded-4" style="background: #fff1f2; border: 1px dashed #fecaca;">
                                                <div class="d-flex justify-content-between align-items-center">
                                                    <div>
                                                        <span class="text-uppercase text-danger opacity-75 fw-bold mb-1 d-block" style="font-size: 0.7rem;">ជំពាក់នៅសល់ (REMAINING)</span>
                                                        <span id="summary-remaining-after" class="h3 mb-0 fw-black text-danger">$0.00</span>
                                                    </div>
                                                    <div class="bg-white rounded-circle shadow-sm text-danger d-flex align-items-center justify-content-center" style="width: 48px; height: 48px;">
                                                        <i class="fi fi-rr-clock-three fs-5"></i>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="v5-receipt-zig-zag" style="height: 12px; background: radial-gradient(circle at 10px 10px, transparent 10px, #ffffff 11px) repeat-x -10px -10px; background-size: 20px 20px;"></div>
                                </div>

                                <!-- ACTIONS -->
                                <div class="v5-actions mt-4">
                                    <div class="glass-toggle p-3 mb-3 rounded-4 border d-flex justify-content-between align-items-center bg-white shadow-sm">
                                        <div class="d-flex align-items-center gap-3">
                                            <div class="v5-mini-icon bg-primary bg-opacity-10 text-primary rounded-3 d-flex align-items-center justify-content-center" style="width: 36px; height: 36px;"><i class="fi fi-rr-print"></i></div>
                                            <span class="fw-bold small text-dark">បោះពុម្ពស្វ័យប្រវត្តិ (Auto-print)</span>
                                        </div>
                                        <div class="form-check form-switch custom-switch-v5 m-0 fs-5">
                                            <input class="form-check-input" type="checkbox" id="autoPrintToggle" checked>
                                        </div>
                                    </div>
                                    
                                    <div class="d-flex flex-column gap-2">
                                        <button type="button" class="btn btn-primary w-100 fw-black fs-6 shadow py-3 rounded-4 border-0 d-flex justify-content-center align-items-center animate__animated animate__pulse animate__infinite animate__slow" onclick="saveAdditionalPayment('${key}')" style="background: linear-gradient(to right, #2563eb, #1d4ed8);">
                                            <i class="fi fi-rr-disk me-2"></i>រក្សាទុកទិន្នន័យ (SAVE)
                                        </button>
                                        <button type="button" class="btn btn-light w-100 fw-bold py-3 rounded-4 border" data-bs-dismiss="modal">
                ...
                <style>
                    ...
                    .badge-status-glow { 
                        ...
                        background: #f8fafc; color: #64748b; padding: 8px 16px; border-radius: 50px; 
                        font-size: 0.75rem; font-weight: 800; border: 1px solid #e2e8f0;
                        transition: 0.3s;
                    }
                    .badge-status-glow.active-full { background: #f0fdf4 !important; color: #166534 !important; border-color: #86efac !important; box-shadow: 0 0 15px rgba(16, 185, 129, 0.2); }
                    .badge-status-glow.active-partial { background: #eff6ff !important; color: #1e40af !important; border-color: #bfdbfe !important; }
...
function saveAdditionalPayment(key) {
    ...
    // Calculate the paid months based on user's manual input
    const modal = document.getElementById('additionalPaymentModal');
    const prevTotalMonths = parseFloat(modal?.dataset.paymentMonths) || 0;
    const months = parseFloat(totalMonths) || 0;
    const currentTotalMonths = prevTotalMonths + months;
    ...
    if (currentTotalMonths) updateData.paymentMonths = currentTotalMonths;
    if (receiptUrl) updateData.attachments = studentAttachments;
    updateData.paymentStatus = (payStatus === 'Delay') ? 'Delay' : ((newRemaining <= 0) ? 'Paid' : 'Installment');
    ...
    studentsRef.child(key).update(updateData)
        .then(async () => {
            if (allStudentsData[key]) {
                ...
                // Dynamically update the Debt Report Table if it is visible
            ...
            try {
                ...
                const chineseName = (s.chineseLastName || '') + ' ' + (s.chineseFirstName || '');
                
                let appUrl = window.location.origin;
                if (appUrl.includes('file://') || appUrl === 'null' || appUrl.includes('localhost') || appUrl.includes('127.0.0.1')) {
                    appUrl = 'https://tx0000-dea44.firebaseapp.com'; // Fallback to hosted domain
                }
                const receiptLink = `${appUrl}/data-tracking.html?print=${key}&index=${newIndex}`;

                if (typeof buildAndSendTelegramMessage === 'function') {
                    buildAndSendTelegramMessage({
                        title: `💰 ការបង់ប្រាក់បន្ថែម (ប្រចាំខែ${newInstallment.forMonth || '----'}) (Additional Payment)`,
                        data: {
                            "អត្តលេខ (ID)": s.displayId || 'N/A',
                            "ឈ្មោះ (Name)": `${s.lastName || ''} ${s.firstName || ''}`.trim(),
                            "ឈ្មោះចិន (Chinese Name)": chineseName.trim() || 'N/A',
                            "កម្រិតសិក្សា (Level)": s.studyLevel || 'N/A',
                            "ម៉ោងសិក្សា (Time)": s.studyTime || 'N/A',
                            "គ្រូបន្ទុកថ្នាក់ (Teacher)": s.teacherName || 'N/A',
                            "កាលបរិច្ឆេទបង់ប្រាក់ (Date)": newInstallment.date,
                            "ម៉ោងដែលបានបង់ (Time Paid)": currentTime,
                            "ទឹកប្រាក់ (Amount)": `$${newInstallment.amount}`,
                            "ប្រាក់ជំណាក់ (Boarding)": `$${newInstallment.boardingFee || 0}`,
                            "រយៈពេលសិក្សា (Duration)": `${newInstallment.months} ខែ`,
                            "ថ្ងៃត្រូវបង់ប្រាក់បន្ទាប់ (Next Due Date)": nextDueDate || 'N/A',
                            "ដំណើរកាល (Stage)": newInstallment.stage,
                            "អ្នកទទួល (Receiver)": newInstallment.receiver
                        },
                        buttons: [
                            { text: "📥 Download PDF (របាយការណ៍)", url: receiptLink }
                        ]
                    });
                }
            } catch (err) { console.error('Telegram Error:', err); }

            // Fix ARIA hidden focus issue before hiding modal
...
// URL Parameter printing handler for Telegram button
window.addEventListener('DOMContentLoaded', () => {
    const urlParams = new URLSearchParams(window.location.search);
    const printKey = urlParams.get('print');
    const index = urlParams.get('index');
    if (printKey) {
        // Wait for Firebase data to load before printing
        let checkAttempts = 0;
        const waitForData = setInterval(() => {
            checkAttempts++;
            if (typeof window.allStudentsData !== 'undefined' && window.allStudentsData[printKey]) {
                clearInterval(waitForData);
                if (typeof window.printPaymentReceipt === 'function') {
                    window.printPaymentReceipt(printKey, index ? parseInt(index) : 0);
                }
            } else if (checkAttempts > 20) { // Stop after 10 seconds (20 * 500ms)
                clearInterval(waitForData);
                console.warn('Could not load student data for PDF printing within 10 seconds.');
            }
        }, 500);
    }
});
